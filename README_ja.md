...
- **対話 → 世界ルールが自律的に更新される**（Three.jsで光・波紋・共鳴が変化）
- **対話 → 新しい存在が生成され、世界に定着する**（Nano Banana）
- **共有世界 → 他者の対話が世界を育てる**（世界状態を同期＝合意形成）

> “世界が自律的に変わり続ける瞬間”を、みんなが同じ世界として目撃できるライフゲームの形。

---

## コンセプト

**EGO** は「対話＝世界パッチ」の2Dドット・ライフゲーム（神視点）です。

Gemini 3 が対話を **構造化された「世界パッチ(JSON)」** に変換し、Three.js 上の世界法則（環境エフェクト）を自律的に更新します。
同時に Nano Banana が新しいドット絵アセットを生成し、世界に定着させます。
...
| カテゴリ                | 採用技術                                                                                         | 用途                                                       |
| ----------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------- |
| モノレポ/パッケージ管理 | pnpm workspace, Node.js 20+                                                                      | `packages/client` と `packages/contracts` の一元管理       |
| フロントエンド          | React 18, Vite 4, TypeScript 5, `@react-three/fiber`                                             | UI描画、3D表現、クライアント実装                           |
| ブロックチェーン同期    | Lattice MUD (`@latticexyz/*`), viem                                                              | World状態の同期、RECS連携、クライアント接続                |
| スマートコントラクト    | Solidity 0.8.24, Foundry, MUD CLI                                                                | World定義、System実装、デプロイ/テスト                     |
| 開発/運用補助           | mprocs, ESLint, Prettier, solhint                                                                | 並列開発起動、品質チェック、整形/静的解析                  |
| 生成AI連携（設計）      | Gemini 3（JSON Mode）, Nano Banana, ADK Orchestrator, Cloud Run, Artifact Registery, ADK, Python | 音声から世界パッチ生成、アセット生成、オーケストレーション |
...
> **「世界が自律的に変わり続ける瞬間」を、みんなが同じ世界として目撃できる（共有＝合意形成）**
...
3. 右上に **パッチJSON** が表示され、世界の **光/波紋/共鳴** が自律変化する
4. 画像生成エージェントの生成物が世界に定着する
5. （可能なら決定打）別タブ/別端末で同じ変化が共有されていることを見せる
...
├─ sample/
│  ├─ fullstack-voice-ai-agent/. # 参考実装（ADK・リアルタイム音声AIエージェント）
